            ...
            with st.expander(f"👤 {cli['nome']} {colab_txt}"):
                ...
                for p in procs:
                    with st.container(border=True):
                        ...
                        # Tratamento para evitar erro se o status no banco não estiver na lista
            ...
            if proc_res.data:
                ...
                if st.button("Gerar Contrato"):
                    if valor_total <= 0:
                        ...
                        # Lançar entrada no caixa se houver
                        if valor_entrada > 0:
                             supabase.table('caixa').insert({
...
def main():
    if 'usuario' not in st.session_state:
        # TELA DE LOGIN (SEM CSS DOS BOTÕES GRANDES)
